...
정규화 장치 좌표(NDC)로 변환된 좌표들은 이후에 우리가 `glViewport` 함수로 지정한 데이터를 이용해 뷰포트 변환(Viewport Transform)을 거쳐 스크린 좌표(Screen-space coordinates)로 변환됩니다. 
...
버퍼에도 고유한 ID가 부여됩니다. 따라서 `glGenBuffers`를 사용해 버퍼 ID를 생성할 수 있습니다. 
...
우리는 새롭게 생성된 버퍼를 <Blue>GL_ARRAY_BUFFER</Blue> 타겟으로 전달해 줄수 있습니다. 바로 `glBindBuffer` 함수를 사용해서 말이죠.
...
를 설정하게 됩니다. 그 다음 `glBufferData` 함수를 호출해서 미리 정의한 정점 배열을 현재 바인딩된 버퍼의 메모리로 복사합니다.

```cpp

glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
```
`glBufferData`는 사용자 정의 데이터를 현재 바인딩된 버퍼에 복사하는 데 특화된 함수입니다. 
<NumberedList>
...
우리는 생성하고자하는 타입의 쉐이더를 `glCreateShader` 함수의 인자로 전달합니다. 정점 쉐이더(vertex shader)를 만들 것이므로 우리는 <Blue>GL_VERTEX_SHADER</Blue>를 넘겨줍니다. 
...
`glShaderSource` 함수는 첫번째 인자로 컴파일할 쉐이더 객체를 받습니다. 두번째 인자는 몇개의 string을 소스코드로 넘겨줄 것인지를 지정합니다. 현재는 한개만 
넘겨주므로 1로 설정합니다. 세번째 인자는 정점 쉐이더의 실제 소스 코드를 넘겨줍니다. 그리고 네번째 인자는 `NULL`로 둡니다. 

이제 `glCompileShader`를 호출하여 쉐이더를 컴파일합니다.

<Info>
아마도 당신은 이제 `glCompileShader`를 호출 후에 컴파일이 성공했는지 아닌지 확인하고 싶을 것입니다. 
...
`glGetShaderiv` 를 사용하여 컴파일이 성공했는지 확인합니다. 만약 컴파일이 실패했다면,`glGetShaderInfoLog`를 호출해 오류 메세지를 가져오고 출력해야 합니다. 
...
이전에 컴파일된 쉐이더들을 프로그램 객체에 붙이고 `glLinkProgram` 
으로 이들을 연결합니다. 
...
코드는 매우 직관적입니다. 우리는 쉐이더들을 프로그램에 붙인 뒤에  `glLinkProgram` 을 통해 그것들을 연결합니다. 


<Info>
Shader 컴파일때처럼 우리는 쉐이더 프로그램의 링킹의 실패도 체크해야합니다. 그리고 에러 로그를 반환해야 합니다. 
  `glGetShaderiv`  와 `glGetShaderInfoLog` 대신 이번에는 `glGetProgramiv`  와 `glGetProgramInfoLog` 를 사용합니다. 
...
```
 ...
 `glUseProgram` 이후의 모든 쉐이더와 렌더링 콜은 이제 이 프로그램 객체를 사용할 것입니다. 
...
이 정보를 바탕으로, 우리는 `glVertexAttribPointer`를 사용하여 OpenGL에게 정점 데이터를 정점 속성 단위로 어떻게 해석해야 하는지 알려줄 수 있습니다. 
...
`glVertexAttribPointer` 함수는 꽤 몇가지의 매개변수들을 가지고 있습니다. 그러니 천천히 그것들을 살펴 봅시다. 
...
<Info>
각 정점 속성(Vertex Attribute)는 VBO가 관리하는 메모리에서 데이터를 가져오며 어떤 VBO에서 데이터를 가져올지는 (여러개의 VBO를 가지고 있을 수 있으므로로) `glVertexAttribPointer`를 호출할때 <Blue>GL_ARRAY_BUFFER</Blue>에 현재 바인딩된 VBO에 의해 결정됩니다. 즉 우리가 <Red>glVertexArrayPointer</Red>를 
...
이제 OpenGL이 정점 데이터를 어떻게 해석해야하는지 지정했으니, `glEnableVertexAttribArray`를 사용해 정점 속성을 활성화 해야합니다. 이 함수에는 정점 속성의 location(슬롯) 값을 인자로 전달합니다. (정점 속성은 기본적으로 비활성화 상태입니다.)
...
- `glEnableVertexAttribArray` 또는 <Red>glDisableVertexAttribArray</Red> 호출
- `glVertexAttribPointer` 를 통한 정점 속성 설정
...
```
VAO를 사용하기 위해서는 VAO를 `glBindVertexArray` 를 이용해 바인딩해야합니다. 
...
## 드디어 기다렸던 삼각형!
  ...
  우리가 원하는 객체를 그리기 위해 OpenGL은 `glDrawArrays` 함수를 제공합니다. 이 함수는 현재 활성화된 쉐이더, 
...
보시다시피 지정된 정점들 사이에 중복이 있습니다. 오른쪽 아래 정점과 왼쪽쪽 위 정점을 두번씩 지정하고 있습니다. 
...
VBO와 유사하게 EBO는 인덱스들을 버퍼에 `glBindBuffer` 를 사용하여 복사할 수 있습니다. 또한 VBO와 유사하게 우리는 바인딩하는 사이에 이 콜을 넣고 ,혹은 언바인딩 할수도 있습니다. 
...
마지막으로 해야 할 일은  `glDrawArrays` 호출을 `glDrawElements`로 바꾸는 것입니다. 
이는 인덱스 버퍼에서 삼각형을 렌더링하고 싶다는 것을 나타냅니다.`glDrawElements` 사용할 때는 
...
첫번째 인자는 `glDrawArrays`와 마찬가지로, 우리가 어떤 모드로 그릴지를 지정합니다. 두 번째 인자는 그릴 요소(element)의 개수로
...
`glDrawElements` 함수는 <Blue>GL_ELEMENT_ARRAY_BUFFER</Blue> 타깃에 바인딩된 EBO(Element Buffer Object)로 부터 인덱스를 
...
VAO는 타겟이 <Blue>GL_ELEMENT_ARRAY_BUFFER</Blue> 일때 `glBindBuffer` 콜을 저장해둡니다. 이는 VAO를 언바인드 하기 전에 EBO를 먼저 언바인드 하지 않도록 주의해야함을 의미합니다. 
...
삼각형을 와이어프레임 모드로 그리려면, `glPolygonMode`을 사용해 OpenGL이 프리미티브를 그리는 방식을 설정할 수 있습니다. 
...
<NumberedList>
<NumberedItem>데이터에 더 많은 정점을 추가하여 `glDrawArrays` 를 사용하여 나란히 있는 2개의 삼각형을 그려보세요: <LinkBlock url="https://learnopengl.com/code_viewer_gh.php?code=src/1.getting_started/2.3.hello_triangle_exercise1/hello_triangle_exercise1.cpp" text="해답" />.</NumberedItem>
