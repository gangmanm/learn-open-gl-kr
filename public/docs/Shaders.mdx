...
우선 우리는 `glfwGetTime`을 사용해 실행 시간을 초 단위로 가져옵니다. 그런다음 <Red>sin</Red> 함수를 사용해 색상을 0.0 ~ 1.0 범위에서 변화시키고 
그 결과를 <Blue>greenValue</Blue>에 저장합니다. 그 다음 `glGetUniformLocation`을 사용해서 uniform <Blue>ourColor</Blue>의 위치를 조회합니다.
이 조회 함수에는 쉐이더 프로그램과 위치를 알고싶은 uniform의 이름을 전달합니다. 만약 `glGetUniformLocation`이 `-1`을 반환한다면, 
해당 uniform의 위치를 찾을 수 없다는 뜻입니다.

마지막으로 `glUniform4f` 함수를 이용해 uniform 값을 설정할 수 있습니다. 주의할 점은 uniform 위치를 찾을 때는 쉐이더 프로그램을 먼저 활성화 할 필요가 없지만, uniform 값을 업데이트 할 때는 반드시 `glUseProgram`을 호출해 프로그램을 활성화 해야 한다는 것입니다. 왜냐면 uniform 값은 현재 활성화 된 쉐이더 프로그램에 적용되기 때문입니다.



<Info> 
OpenGL은 근본적으로 C라이브러리이기 때문에, 함수 오버로딩을 네이티브로 지원하지 않습니다. 따라서 하나의 함수가 여러 타입으로 호출될 수 있는 경우,
OpenGL은 필요한 타입마다 새로운 함수를 정의합니다. `glUniform4f`이 그 예시입니다. 
...
우리의 경우 uniform에 4개의 float 값을 각각 전달하려고 하므로 `glUniform4f`를 사용합니다.(참고로 `fv` 버전을 사용할 수도 있습니다.)
...
현재의 레이아웃을 바탕으로 우리는 정점 형식을 `glVertexAttribPointer`로 업데이트할 수 있습니다. 
...
`glVertexAttribPointer`의 앞쪽 몇몇 인자들은 비교적 직관적입니다. 이번에는 속성 위치(location) 1에 정점 속성을 설정합니다. 
